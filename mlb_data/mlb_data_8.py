# 류현진과 커쇼의 시즌별 평균자책점을 비교한 그래프 시각화 개선
...
with sqlite3.connect("lahmansbaseballdb.sqlite") as con:
    # 커서 객체 생성 
    cur = con.cursor()
    # SQL문 (투구 테이블에서 커쇼와 류현진의 정보(선수ID, 연도, 팀, 평균자첵점) 조회) 실행
    cur.execute('''
